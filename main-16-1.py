...
def build_result(page, navigate):
    ...
    def _score_bar(label: str, score: float) -> ft.Control:
        ...
        # colour: red ≥70 %, amber 40–70 %, grey below 40
        if pct >= 0.70:
            bar_color = P.ERR
        elif pct >= 0.40:
            bar_color = P.WARN
        else:
            bar_color = P.BORDER_STRONG   # neutral grey for low scores
    ...
    def _build_profanity_result(pr: dict) -> ft.Control:
        ...
        # ── Colours driven purely by is_profane (red spectrum only) ──────────
        if err:
            accent = P.MUTED
        elif is_profane:
            accent = P.ERR
        ...
        hero_banner = ft.Container(
            ft.Row([
                ft.Container(
                    ft.Icon(hero_icon, color=P.WHITE, size=32),
                    width=56, height=56,
                    bgcolor="#FFFFFF26",     # translucent white circle behind icon
                    border_radius=18,
                    alignment=ft.alignment.Alignment(0, 0),
                ),
        ...
        stat_row = ft.Container(
            ft.Row([
                ...
                _stat("Confidence", f"{score_pct:.1f}%",
                      P.ERR if score_pct >= 70 else (P.WARN if score_pct >= 40 else P.ERR_MUTED)),
